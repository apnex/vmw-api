...
module.exports = class vmwApi {
	...
	async clear() {
		...
		try {
			...
			fs.unlinkSync('./fileList.json');
			fs.unlinkSync('./headers.json');
			fs.unlinkSync('./mainIndex.json');
		} catch(error){}
		console.log('CLEAR: all state cleared');
	}
	async tryAuth() {
		// check cache if username has changed since last attempt - if so, clear cache
                let value = await cache.getItem('username');
		if(value != this.username) {
			console.log('[WARN]: AUTH-CHANGE - [' + value + ' : ' + this.username + ']');
			await this.clear();
			await cache.setItem('username', this.username);
                }
		// attempt accountInfo
		try {
	...
	async getDownload(fileName) {
		...
		if(fs.existsSync('./fileList.json')) {
