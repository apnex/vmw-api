...
module.exports = class vmwClient {
				...
				afterResponse: [
					(response, retryWithMergedOptions) => {
						if (response.statusCode === 401) { // Unauthorized
							// THROW AN ERROR!!!
							let error = new Error('[ERROR]: Session timed out - Please reauthenticate', 400);
							error.code = 401;
							throw error;
							...
							const updatedOptions = {
								headers: {
									'X-XSRF-TOKEN': await this.getNewToken(this.options.username, this.options.password) // Refresh the access token
								}
							};
	...
	async login(options) {
		...
		// what response do I get if missing
		// write to file
		let headers;
